...
async function get(url, id, bkp) {
    try {
        ...
        await fsp.writeFile(`files/temp/${id}.json`, JSON.stringify({ id: id, type: "raid-battles", data: raidboss }));
    } catch (err) {
        console.error(err);
        for (var i = 0; i < bkp.length; i++) {
            if (bkp[i].eventID == id) {
                await fsp.writeFile(`files/temp/${id}.json`, JSON.stringify({ id: id, type: "raid-battles", data: bkp[i].extraData.raidbattles.data }));
            }
        }
    }
}
