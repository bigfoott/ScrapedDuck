...
async function get(url, id, bkp) {
    try {
        ...
        {
            await fsp.writeFile(`files/temp/${id}.json`, JSON.stringify({ id: id, type: "community-day", data: commday }));
        }
    } catch (err) {
        console.error(err);
        for (var i = 0; i < bkp.length; i++) {
            if (bkp[i].eventID == id && bkp[i].extraData != null) {
                await fsp.writeFile(`files/temp/${id}.json`, JSON.stringify({ id: id, type: "community-day", data: bkp[i].extraData.communityday.data }));
            }
        }
    }
}
